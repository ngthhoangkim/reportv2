...
async function convertDocxToPdf(docxPath, pdfPath) {
  return enqueue(async () => {
    logger.job('info', 'word convert docx to pdf started', { docxPath, pdfPath });
    const result = await convertWithWord(docxPath, pdfPath, 'pdf');
    logger.job('info', 'word convert docx to pdf completed', { docxPath, pdfPath });
    return result;
  });
}
...
async function convertDocToDocxCached(docPath) {
  ...
  return enqueue(async () => {
    logger.job('info', 'word convert doc to docx started', { docPath, cached });
    const result = await convertWithWord(docPath, cached, 'docx');
    logger.job('info', 'word convert doc to docx completed', { docPath, cached });
    return result;
  });
}
